...
function runViteDev() {
  ...
  const viteConfig = path.resolve(process.cwd(), 'frontend', 'vite.config.ts');
  return runSpawn(process.execPath, [viteEntry, '--config', viteConfig, '--port=3000', '--host=0.0.0.0'], 'vite dev');
}

function runPythonPlatform() {
  return runSpawn('python', ['backend/python/run_server.py'], 'python platform service');
}
...
function watchPythonPlatformSources() {
  const roots = [
    path.resolve(process.cwd(), 'backend', 'python'),
  ];
  ...
  for (const root of roots) {
    try {
      const watcher = watch(root, { recursive: true }, (_eventType, filename) => {
        ...
        if (changed.startsWith('data/')) return;
        if (changed.startsWith('strategy_store/')) return;
        if (changed.startsWith('app/data/')) return;
