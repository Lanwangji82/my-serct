...
import type { AssetRow, BacktestLog, BacktestRun, BacktestTrade, MarketEvent, MarketRow, PlatformStrategy } from "./strategy-types";
...
const toChartTime = (time: number) => Math.floor(time / 1000) as UTCTimestamp;
...
function ChartPanel(props: { candles: Candle[]; hovered: Candle | null; onHover: (time: number | null) => void }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(el, {
      ...
      crosshair: {
        ...
        vertLine: { color: "rgba(161,161,170,0.4)", width: 1, style: 3, labelBackgroundColor: "#18181b" },
        horzLine: { color: "rgba(161,161,170,0.4)", width: 1, style: 3, labelBackgroundColor: "#18181b" },
      },
      rightPriceScale: { borderColor: "rgba(63,63,70,0.55)", scaleMargins: { top: 0.04, bottom: 0.18 } },
      timeScale: { borderColor: "rgba(63,63,70,0.55)", rightOffset: 10, barSpacing: props.candles.length > 180 ? 9 : 12, minBarSpacing: 6, timeVisible: true, secondsVisible: false },
    ...
    const volumeSeries = chart.addSeries(HistogramSeries, { priceFormat: { type: "volume" }, priceScaleId: "", lastValueVisible: false, priceLineVisible: false }, 1);

    candleSeries.setData(props.candles.map((c): CandlestickData<Time> => ({ time: toChartTime(c.time), open: c.open, high: c.high, low: c.low, close: c.close })));
    volumeSeries.setData(props.candles.map((c): HistogramData<Time> => ({ time: toChartTime(c.time), value: Math.max(c.volume, 0), color: c.close >= c.open ? "rgba(16,185,129,0.28)" : "rgba(244,63,94,0.28)" })));

    const markers = props.candles.flatMap((candle, i) => candle.events.map((event, j) => {
      const marker = (event.marker || event.label || "").toLowerCase();
      const isShort = marker.includes("short");
      const isClose = marker.includes("close");
      return { id: `${i}-${j}-${event.id || marker}`, time: toChartTime(candle.time), position: isShort ? ("aboveBar" as const) : ("belowBar" as const), shape: isClose ? ("circle" as const) : isShort ? ("arrowDown" as const) : ("arrowUp" as const), color: isShort ? "#f43f5e" : "#22c55e", text: event.marker || event.label || (isShort ? "short" : "long") };
    }));
    const markerApi = createSeriesMarkers(candleSeries, markers);

    const lookup = new Map(props.candles.map((c) => [toChartTime(c.time), c.time] as const));
    const handleMove = (param: MouseEventParams<Time>) => props.onHover(typeof param.time === "number" ? lookup.get(param.time as UTCTimestamp) ?? null : null);
  ...
  return (
    <div className="grid gap-3">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-xl border border-zinc-800 bg-zinc-950/40 px-3 py-2 text-xs">
        <span className="text-zinc-500">{props.hovered ? formatDateTime(props.hovered.time) : "--"}</span>
      ...
      <div className="relative">
        ...
        <div ref={ref} className="h-[640px] w-full overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950/40" />
      </div>
    </div>
  );
}
